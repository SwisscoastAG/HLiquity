...
contract HCHFToken is IHCHFToken, HederaTokenService, ExpiryHelper, KeyHelper, CheckContract
{
    ...
    string constant internal _NAME = "HCHF Stablecoin";
    string constant internal _SYMBOL = "HCHF";
 ...
 * @dev Returns the name of the token
    ...
    function balanceOf(
        address account
    ) external view override(IHCHFToken) returns (uint256) {
 ...
 * @dev Returns the total number of tokens that exits
    ...
    function _requireCallerIsBOorTroveMorSP() internal view {
        require(
            ...
            "HCHF: Caller is neither BorrowerOperations nor TroveManager nor StabilityPool"
        );
    }

    function _requireCallerIsStabilityPool() internal view {
        require(msg.sender == stabilityPoolAddress, "HCHF: Caller is not the StabilityPool");
    }

    function _requireCallerIsTroveMorSP() internal view {
        require(
            msg.sender == troveManagerAddress || msg.sender == stabilityPoolAddress,
            "HCHF: Caller is neither TroveManager nor StabilityPool");
    }
}